...
export const updateAddressById = async (
  ...
  try {
    ...
    const { name, street, landmark, city, state, mobileNo, postalCode } =
      req.body;
...
// Remove user address
export const removeAddressById = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void | Response> => {
  try {
...
export const resendSecurityCode = async (
  req: Request,
  res: Response
): Promise<void | Response> => {
  try {
    ...
    const now = new Date();
    const isCodeValid =
      user.securityCodeExpires && user.securityCodeExpires > now;
